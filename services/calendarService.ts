  ...
  allEvents.forEach((event: Schema$Event & { branch: string }) => {
    try {
      ...
      batch.set(eventRef, {
        ...
        startTime: validEvent.start.dateTime ? new Date(validEvent.start.dateTime).toISOString() : validEvent.start.date,
        endTime: validEvent.end.dateTime ? new Date(validEvent.end.dateTime).toISOString() : validEvent.end.date,
